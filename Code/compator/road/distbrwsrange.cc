/* ----------------------------------------------------------------------------
    ...
    This program performs range search based on distance browsing index.

    Suggested arguments:
    > (prog name) -i graph.idx -o object.dat -s query.dat -x distbrws.idx -r range -v
    explanations:
...
void helpmsg(const char* pgm)
{
    ...
    cerr << "> " << pgm << " ";
    cerr << "-i graph.idx -o object.dat -s query.dat -x quadtrees.idx -k #NNs -v" << endl;
    cerr << "explanations:" << endl;
    cerr << "-i: graph index file" << endl;
    cerr << "-o: object file" << endl;
    cerr << "-s: query file" << endl;
...
int main(const int a_argc, const char** a_argv)
{
    ...
    const float r = (float)atof(Param::read(a_argc, a_argv, "-r", ""));
    ...
    while (true)
    {
        ...
        {
            ObjectSearchResult* res = (ObjectSearchResult*)result.get(i);
            delete res;
        }
    }
    fquery.close();
